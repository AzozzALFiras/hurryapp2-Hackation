...
<span class="app-brand-text demo menu-text fw-semibold ms-2">3Sala</span>
</a>
...
<x-db.menu-item icon="ri-home-smile-line" itemName="لوحة التحكم" active="{{ request()->routeIs('home.*') ? 'true' : 'false' }}">
<x-db.menu-sub>
<x-db.collapsed-menu-item itemName="الاحصائيات" url="{{ route('home.index') }}"  active="{{ request()->routeIs('home.index') ? 'true' : 'false' }}"/> 
  <x-db.collapsed-menu-item itemName="الاعدادات" url="{{ route('home.settings') }}"  active="{{ request()->routeIs('home.settings') ? 'true' : 'false' }}"/> 

</x-db.menu-sub>
</x-db.menu-item>

{{-- 
<x-db.menu-item icon="ri-layout-2-line" itemName="sections" active="{{ request()->routeIs('section.*') ? 'true' : 'false' }}">
<x-db.menu-sub>
<x-db.collapsed-menu-item itemName="section" url="{{ route('section.index') }}"  active="{{ request()->routeIs('section.index') ? 'true' : 'false' }}"/> 
<x-db.collapsed-menu-item itemName="add" url="{{ route('section.create') }}" active="{{ request()->routeIs('section.create') ? 'true' : 'false' }}" />
</x-db.menu-sub>
</x-db.menu-item>

<x-db.menu-item icon="ri-layout-2-line" itemName="digitalProducts" active="{{ request()->routeIs('license.*') ? 'true' : 'false' }}">
<x-db.menu-sub>
<x-db.collapsed-menu-item itemName="digitalProducts" url="{{ route('license.index') }}"  active="{{ request()->routeIs('license.index') ? 'true' : 'false' }}"/> 
<x-db.collapsed-menu-item itemName="add" url="{{ route('license.create') }}" active="{{ request()->routeIs('license.create') ? 'true' : 'false' }}" />
</x-db.menu-sub>
</x-db.menu-item> --}}
